...
#' @param target char to determine the assumption used to combine
...
#'
calc_new_coverage <- function(coverage, prev_immunity, target) {
...
assert_valid_target <- function(x, name = deparse(substitute(x))) {
    assert_character(x)
