test_that("coverage_from_doses returns the correct coverage for sensible inputs", {
    ...
    expect_equal(0.25, coverage_from_doses(pop_df = pop, doses = 50,
                                           region = "UK", year = 2001,
                                           age_first = 0, age_last = 1))

    ## more doses than the target population:
    expect_equal(1, coverage_from_doses(pop_df = pop, doses = 500,
                                        region = "UK", year = 2001,
                                        age_first = 0, age_last = 1))

    ## targeting several regions at once:
    expect_equal(0.5, coverage_from_doses(pop_df = pop, doses = 400,
                                          region = c("UK", "FRA"),
                                          year = 2001, age_first = 0,
                                          age_last = 3))

    ## using the default age_first/age_last entries
    expect_equal(0.25, coverage_from_doses(pop_df = pop, doses = 100,
                                           region = "UK", year = 2001))
})
...
test_that("coverage_from_doses fails if the population object doesn't contain all targeted cohorts", {
    ...
    expect_error(coverage_from_doses(pop_df = pop, doses = 100, region = "GER",
                                     year = 2001, age_first = 0, age_last = 100))
    ## year not covered in pop:
    expect_error(coverage_from_doses(pop_df = pop, doses = 100, region = "UK",
...
test_that("coverage_from_doses fails for silly input values", {
    ...
    expect_error(coverage_from_doses(pop_df = pop, doses = 50, region = "UK",
