test_that("apply_vacc correctly uses apply_vacc for a single vacc_activity", {
    ...
    ## applying a single vacc activity - does apply_vacc give the
    ## same as apply_vacc?
    pop_sing <- apply_vacc_1(pop = pop, region = "UK", year = vacc$year,
                           age_first = vacc$age_first,
                           age_last = vacc$age_last,
                           coverage = vacc$coverage,
                           doses = NA,
                           targeting = vacc$targeting)
    pop_multi <- apply_vacc(pop = pop, vacc = vacc)
    
    expect_equal(pop_sing, pop_multi)
})
test_that("apply_vacc gives the same result as apply_vacc_1 used twice for two activities", {
    ## get a population object
    pop <- popim_population(region = "UK", year_min = 2000, year_max = 2010,
                          age_min = 0, age_max = 10)
    ## get a vaccine object
    ...
    pop_sing <- apply_vacc_1(pop = pop, region = "UK", year = vacc$year[1],
                           age_first = vacc$age_first[1],
                           age_last = vacc$age_last[1],
                           coverage = vacc$coverage[1],
                           doses = vacc$doses[1],
                           targeting = vacc$targeting[1])
    pop_sing <- apply_vacc_1(pop = pop_sing, region = "UK", year = vacc$year[2],
